		...
		unsigned long arg)
{
	...
	case SCULL_IOCKQUANTUM: /* Return task_info struct */
		;
		struct task_info ti = {
			.state = current->state,
			.stack = current->stack,
		...
		retval = copy_to_user(arg, &ti, sizeof(ti));
		...
		if (head == NULL) {
			...
			if (tail->pid == p->pid && tail->tgid == p->tgid) {
...
void scull_cleanup_module(void)
{
	...
	//printk("Testing: headPID = %i, nextPID = %i, nextnextPID = %i\n", head->pid, head->next->pid, head->next->next->pid);

	printk(KERN_INFO "Linked List to be deleted: \n");
	while (head != NULL) {
		if (head->next == NULL) {
			printk(KERN_CONT "Task %i: PID: %i; TGID: %i\n", counter, head->pid, head->tgid);
		}
		else {
			printk(KERN_CONT "Task %i: PID: %i; TGID: %i -> \n", counter, head->pid, head->tgid);
		}
